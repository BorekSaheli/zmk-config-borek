/*
 * Central side of the image-sync service: runs an hourly timer, updates
 * its own displayed image, and pushes the peripheral's index over a
 * custom GATT characteristic on the split connection.
 *
...
static uint8_t char_discover_cb(struct bt_conn *conn, const struct bt_gatt_attr *attr,
    ...
    if (!attr) {
        LOG_DBG("image_sync char discovery complete (handle=%u)", char_handle);
...
static void connected_cb(struct bt_conn *conn, uint8_t err) {
    ...
    /* We only care about the outgoing connection to the split peripheral. */
